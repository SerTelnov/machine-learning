...
class ClazzInfo():
    def __init__(self, probability, words_msg_count, msg_count, mul):
...
def train(msgs, clazz_fine):
    ...
    for clazz in range(len(clazz_fine)):
        ...
        if x != 0:
            preparetion.append(ClazzInfo(math.log(x), words_msg_counts[clazz], clazz_msgs[clazz], 1))
        else:
            preparetion.append(ClazzInfo(-1, words_msg_counts[clazz], clazz_msgs[clazz], 0))

    return preparetion

def classifier(preparetion, alfa, words):
    scores = []
    for clazz in range(len(preparetion)):
        ...
        score = _safe_exp(score) * info.mul
        scores.append(score)
...
for _ in range(int(input())):
    curr_msg = input().split(" ")
    classifier(preparetion, alfa, _safe_parse_msg(curr_msg, 1))
